	...
	char* output_folder)				// output folder
{
	...
	start_time = clock();
	Drusilla_Index* di = new Drusilla_Index();
	di->build(n, d, l, m, B, output_folder, data);
	end_time = clock();
	...
	char* output_folder)				// output folder
{
	...
	// -------------------------------------------------------------------------
	//  c-k-Approximate Furthest Neighbor (c-k-AFN) search via DrusillaSelect
	// -------------------------------------------------------------------------
	int kFNs[] = {1, 2, 5, 10};
	int max_round = 4;
	int top_k = 0;
	...
	Drusilla_Index* di = new Drusilla_Index();
	di->load(output_folder);			// load index of DrusillaSelect
	...
	printf("DrusillaSelect for c-k-AFN Search: \n");
	...
	for (int num = 0; num < max_round; num++) {
		...
		for (i = 0; i < qn; i++) {
			list->reset();
			this_io = di->search(query[i], data_folder, list);
			all_io += this_io;
